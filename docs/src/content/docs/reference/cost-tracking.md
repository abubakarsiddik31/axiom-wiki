...
grep "map" wiki/usage.log       # map operation costs
grep "sync" wiki/usage.log      # sync operation costs
```

## Cost estimates

The `map` and `sync` commands show estimated costs before execution, giving you a chance to cancel before any LLM calls are made.
